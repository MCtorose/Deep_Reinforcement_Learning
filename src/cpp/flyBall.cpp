...
{
    // 初始化运行标志位
    (void) std::memset((static_cast<void*>(&flyBall_M)),0,sizeof(RT_MODEL_flyBall_T));

    {
        ...
        rtsiSetContStateDisabledPtr(&(&flyBall_M)->solverInfo, (boolean_T **)&(&flyBall_M)->contStateDisabled);
    ...
    flyBall_X.y_CSTATE = 0.0;
    flyBall_X.vy_CSTATE = 100.0;
    flyBall_B.y = flyBall_X.y_CSTATE;
    flyBall_B.vy_m = flyBall_X.vy_CSTATE;
    flyBall_B.vy = flyBall_B.vy_m;
...
real_T flyBall::get_t()
{
    return (&flyBall_M)->Timing.t[0];
}

real_T flyBall::get_y()
{
    return flyBall_B.y;
}

real_T flyBall::get_vy()
{
    return flyBall_B.vy;
}

real_T flyBall::get_ay()
{
    return flyBall_B.ay;
}

real_T flyBall::get_reward()
{
    return flyBall_B.reward;
}

real_T flyBall::get_target_y()
{
    return flyBall_P.target_y;
}

real_T flyBall::get_target_vy()
{
    return flyBall_P.target_vy;
}

real_T flyBall::get_target_t()
{
    return flyBall_P.target_t;
}

void flyBall::set_control(real_T control)
{
    flyBall_P.control = control;
}

flyBall::flyBall() : flyBall_B(),
                     flyBall_X(),
                     flyBall_XDis(),
                     flyBall_M()
{
